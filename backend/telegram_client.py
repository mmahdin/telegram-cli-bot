...
class TelegramPanelClient:
    ...
    async def _on_new_message(self, event: Any) -> None:
        ...
        try:
            ...
            if not event.is_private:
                return
            await self._cache_event_dialog(event)
    ...
    async def _cache_event_dialog(self, event: Any) -> None:
        try:
            entity = await event.get_chat()
            if isinstance(entity, User):
                self.storage.upsert_dialog(
                    {
                        "id": entity.id,
                        ...
                        "avatar": self.storage.avatar_url(entity.id),
                    }
                )
        except Exception:
            return
        ...
        async with self._dialog_sync_lock:
            try:
                ...
                for dialog in dialogs:
                    ...
                    if not isinstance(entity, User) or entity.id == me.id:
                        continue
                    ...
                    rows.append(
                        {
                            ...
                            "phone": entity.phone or "",
                            "is_bot": bool(entity.bot),
                            "last_message": self._last_message_preview(last_msg),
    ...
    async def send_photo(
        ...
        msg = await self.client.send_file(entity, upload, caption=caption, reply_to=reply_to, force_document=False)
        formatted = await self.format_message(msg)
        self.storage.upsert_message(formatted, update_dialog=True)
        return formatted
    ...
    async def format_message(self, msg: Message) -> dict[str, Any]:
        chat_id = self._private_chat_id(msg)
    ...
    def _private_chat_id(self, msg: Message) -> Optional[int]:
    ...
    def _display_name(user: User) -> str:
        ...
        return name or user.username or user.phone or str(user.id)
